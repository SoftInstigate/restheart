...
public class AclVarsInterpolator {
     ...
     *
     * <p>Unlike {@link #interpolateBson(MongoRequest, BsonValue)}, a {@code @}-prefixed variable that
     ...
     */
    public static BsonValue interpolateFilter(final MongoRequest request, final BsonValue filter) {
     ...
     * @author Maurizio Turatti {@literal <maurizio@softinstigate.com>}
     */
    public static Optional<String> firstUnboundUserVar(final MongoRequest request, final BsonValue bson) {
        if (bson.isDocument()) {
    ...
    private static BsonValue interpolateBson(final MongoRequest request, final BsonValue bson, final boolean filter) {
        if (bson.isDocument()) {
    ...
    private static BsonValue interpolateString(final MongoRequest request, final String key, final String value, final boolean filter) {
