...
public class MongoRequestPropsInjector {
    ...
    public static void inject(HttpServerExchange exchange) {
        ...
        var __rep = exchange.getQueryParameters().get(REPRESENTATION_FORMAT_KEY);

        // default value
        var rep = MongoServiceConfiguration.get().getDefaultRepresentationFormat();

        if (__rep != null && !__rep.isEmpty()) {
            var _rep = __rep.getFirst();
        ...
        if (request.isTxn()) {
            try {
                ...
                response.setInError(HttpStatus.SC_BAD_REQUEST, "illegal txnId: it must be a number");
                return;
            }
        }
        ...
        var __pagesize = exchange.getQueryParameters().get(PAGESIZE_QPARAM_KEY);
        ...
        if (pagesize < 0 || pagesize > MAX_PAGESIZE) {
            response.setInError(HttpStatus.SC_BAD_REQUEST, "illegal pagesize parameter, pagesize must be >= 0 and <= " + MAX_PAGESIZE);
            return;
        } else {
            request.setPagesize(pagesize);
        }

        var __page = exchange.getQueryParameters().get(PAGE_QPARAM_KEY);
        ...
        var __count = exchange.getQueryParameters().get("count");
        ...
        var keys = exchange.getQueryParameters().get(KEYS_QPARAM_KEY);
        ...
        var filters = exchange.getQueryParameters().get(FILTER_QPARAM_KEY);
        ...
        // default value
        var docIdType = DOC_ID_TYPE.STRING_OID;

        if (__docIdType != null && !__docIdType.isEmpty()) {
            var _docIdType = __docIdType.getFirst();
        ...
        if (!request.isPost()) {
            ...
            try {
                ...
                response.setInError(HttpStatus.SC_BAD_REQUEST, "wrong document id format: not a valid " + docIdType.name(), idide);
                return;
            }
        }

        // get the HAL query parameter
        var __halMode = exchange.getQueryParameters().get(HAL_QPARAM_KEY);
        ...
        if (__halMode == null || __halMode.isEmpty()) {
            ...
            if (exchange.getQueryParameters().get(REPRESENTATION_FORMAT_KEY) == null) {

                request.setRepresentationFormat(MongoServiceConfiguration.get().getDefaultRepresentationFormat());
            }
        }

        // get the shardkey query parameter
        // get and check shardKeys parameter
        var shardKeys = exchange.getQueryParameters().get(SHARDKEY_QPARAM_KEY);
