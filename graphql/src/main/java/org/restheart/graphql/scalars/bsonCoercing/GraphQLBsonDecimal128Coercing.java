    ...
    public BsonDecimal128 parseLiteral(Object input) throws CoercingParseLiteralException {
        if(input instanceof StringValue || input instanceof IntValue || input instanceof FloatValue){
            ...
            if(input instanceof IntValue){
                value = ((IntValue) input).getValue().toString();
            }
            else if (input instanceof FloatValue){
                value = ((FloatValue) input).getValue().toString();
            }

            Decimal128 dec = Decimal128.parse(value);

            if(dec.isNaN()){
                throw new CoercingParseLiteralException(
                        "Expected value to be a number but it was '" + dec.toString() + "'"
                );
            }
