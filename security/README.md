...
- Roles based Authorization with a powerful permission definition language.
- Other Authentication and Authorization methods are supported with plugins.
...
A different example would be an Authentication Mechanism that handles the JWT (JSON Web Token) scheme. In this case user id is defined in a digitally signed JSON object passed by a request header. The Authentication Mechanism would then just verify the token and pass the information to the Identity Manager.

The **Identity Manager** verifies the user information generated by Authentication Mechanism. For instance, the BasicAuthenticationMechanism extracts user credentials from the request in the form of ids and passwords. The IDM can check these credentials against a database or and LDAP server.
