...
/**
 * Proves message fanout from the broker to {@code /mqtt-sse} clients over this test class's own
 * RESTHeart / Mosquitto stack: a published message arriving over SSE, a wildcard filter matching
 * it, two clients on the same filter both receiving it, and one client disconnecting not
 * affecting another that stays connected.
 * <p>
 * Every topic used here is under {@code sensors/}, the only prefix {@code it-overrides.yml}'s
 * {@code mqtt-topic-authorizer} ACL grants the {@code admin} role, and every test uses a topic no
 * other test in this class publishes to: {@code /mqtt-sse} replays a topic's last message to a
 * newly connecting client (documented on {@code MqttSseService}), so two tests sharing a literal
 * topic could see each other's payload.
 * </p>
 *
...
public class MqttSseStreamIT extends MqttITBase {
    ...
    void wildcardFilterMatches() throws Exception {
        ...
        // Not a fixed-count readSseLines(req, 3, 15) here, unlike the other three tests in this
        // class. MqttSseService.onConnect replays one cached event per topic that already has a
        // last-known message and matches the subscribed filter (router.getLastMessages(filter)) -
        // and mqtt-router keeps a standing subscription to "sensors/#" for the whole IT suite (see
        // it-overrides.yml), so by the time this test runs, every sensors/* topic any earlier
        // mqtt IT class published to (sensors/acl-granted, sensors/rest-last-value, sensors/
        // reconnect, and so on) already has a cached last message and gets replayed here too, each
        // as its own three-line id/event/data event, before this test's own live publish below.
        // This test reads until it sees the one payload it actually published, tolerating however
        // many other topics' replayed events arrive first.
        var linesFuture = subscribeAsync(
            () -> readSseLinesUntil(req, line -> line.contains("19.0"), 90, 20));
        ...
        // like this one would otherwise be replayed that message's cached value and could
        // mistake it for this test's own.
        publish("sensors/wildcard-temp", "{\"value\": 19.0}");

        var lines = linesFuture.get(25, TimeUnit.SECONDS);
    ...
    void disconnectingClientDoesNotAffectAnotherThatStaysConnected() throws Exception {
        ...
        // A longer settle than the 1500 ms used for the single-connection cases above: this test
        // opens two SSE connections that must each finish the full request pipeline (security,
        // mqtt-topic-authorizer, subscribe) before the first publish, and the extra margin makes
        // that reliable even when this instance is under load from earlier tests.
        Thread.sleep(2_500);
