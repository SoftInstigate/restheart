...
/**
 * An embedding provider that calls nobody: deterministic vectors of length 3, from
 ...
 */
@RegisterPlugin(
        name = "fakeEmbeddingProvider3d",
        description = "Embedding provider for tests: deterministic vectors of length 3, no network",
        enabledByDefault = false)
public class FakeEmbeddingProvider3d implements Provider<EmbeddingModel> {

    @Override
    public EmbeddingModel get(final PluginRecord<?> caller) {
        return (texts, request) -> FakeVectors.embed(texts, 3);
    }
}
